...
esp_err_t bsp_i2c_init(void)
{
    ...
    if (i2c_master_get_bus_handle(BSP_I2C_NUM, &i2c_handle) == ESP_OK) {
    ...
    if (ret == ESP_ERR_INVALID_STATE) {
        ret = i2c_master_get_bus_handle(BSP_I2C_NUM, &i2c_handle);
    }
    if (ret != ESP_OK) {
        return ret;
    }
...
{
    if (ext_i2c_initialized && ext_i2c_bus_handle != NULL) {
        return ESP_OK;
    }

    if (i2c_master_get_bus_handle(BSP_EXT_I2C_NUM, &ext_i2c_bus_handle) == ESP_OK) {
    ...
    esp_err_t ret = i2c_new_master_bus(&i2c_mst_config, &ext_i2c_bus_handle);
    if (ret == ESP_ERR_INVALID_STATE) {
        ret = i2c_master_get_bus_handle(BSP_EXT_I2C_NUM, &ext_i2c_bus_handle);
    }
    if (ret != ESP_OK) {
        return ret;
    }
...
esp_err_t bsp_i2c_scan()
{
    ...
    printf("scan sys i2c device (port 0, SDA=%d, SCL=%d)\n", BSP_I2C_SDA, BSP_I2C_SCL);
...
bsp_display_type_t bsp_detect_display_type(void)
{
...
esp_err_t bsp_touch_new(const bsp_touch_config_t* config, esp_lcd_touch_handle_t* ret_touch)
{
    ...
    bsp_display_type_t dt = bsp_detect_display_type();
    esp_lcd_panel_io_handle_t tp_io_handle = NULL;
    esp_err_t ret = ESP_OK;

    if (dt == BSP_DISPLAY_TYPE_ST7123 || dt == BSP_DISPLAY_TYPE_ST7121) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_i2c(bsp_i2c_get_handle(), &tp_io_config, &tp_io_handle), TAG, "new ST7123 touch IO failed");
        ret = esp_lcd_touch_new_i2c_st7123(tp_io_handle, &tp_cfg, ret_touch);
    } else {
        esp_lcd_panel_io_i2c_config_t tp_io_config = ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
        tp_io_config.dev_addr                      = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP;  // 更改 GT911 地址
        tp_io_config.scl_speed_hz                  = CONFIG_BSP_I2C_CLK_SPEED_HZ;
        ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_i2c(bsp_i2c_get_handle(), &tp_io_config, &tp_io_handle), TAG, "new GT911 touch IO failed");
